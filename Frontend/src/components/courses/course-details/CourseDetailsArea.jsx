...
const tab_title = ["Overview", "Curriculum", "Instructors", "Reviews"];

const CourseDetailsArea = ({ single_course }) => {
            ...
            <div className="courses__details-content">
              <ul className="courses__item-meta list-wrap">
                <li className="courses__item-tag">
                  <Link to="/course">
                    {single_course?.category ? single_course.category : "Development"}
                  </Link>
                </li>
                <li className="avg-rating">
                  <i className="fas fa-star"></i>
                  {single_course?.rating ? `${single_course.rating} (4.5 Reviews)` : "(No Rating)"}
                </li>
              </ul>
              <h2 className="title">
                {single_course?.title ? single_course.title : "Resolving Conflicts Between Designers And Engineers"}
              </h2>
